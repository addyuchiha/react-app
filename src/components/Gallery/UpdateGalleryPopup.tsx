...
function UpdateGallery({ setState, galleryId, initialData, onSuccess, onUpdated }: Props) {
  const [formData, setFormData] = useState<GalleryFormData>(initialData)
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/gallery/${galleryId}`, {
        method: "PUT",
        headers: {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {validationErrors.description ? (
                ...
                <p className="mt-1 text-sm text-gray-500">{500 - formData.description.length} characters remaining</p>
              )}
            </div>

            <div>
              <label className="block text-gray-700 font-medium mb-2" htmlFor="sessionDate">
                Session Date
              </label>
              <input
                type="date"
